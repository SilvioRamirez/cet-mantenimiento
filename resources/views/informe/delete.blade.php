    ...
    <div class="card border-light mb-3 shadow">
        <div class="card-header bg-success text-white">
            <i class="fa fa-trash-alt"></i>
                    {{ __('Eliminar') }} Informe Técnico
        </div>
        <div class="card-body text-center" >			
            @include('fragment.error')
            @include('fragment.success')
                    
            {!! Form::open(['method' => 'DELETE','route' => ['informe.destroy', $informes->id],'style'=>'display:inline']) !!}
                    
                <h2 class="text-center">¿Está segur@ de eliminar el Informe Técnico <strong>{{ $informes->informe_codigo }}</strong>?</h2>
                <hr>
